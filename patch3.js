...
const matchRegex = /nodesToProcess\.forEach\(textNode => \{[\s\S]*?textNode\.parentNode\?\.replaceChild\(frag, textNode\);\n                    \}\);/;
...
if (matchRegex.test(mainTs)) {
    mainTs = mainTs.replace(matchRegex, replacement);
...
const calRegex = /const \{ isMatch, dateStr, lineNo \} = isDateClickableRange\(view, pos\);\n[\s\S]*?return true;\n                \}/;

if (calRegex.test(calTs)) {
    ...
    calTs = calTs.replace(calRegex, newClickLogic);
