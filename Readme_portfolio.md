...
- **초기화 엔진 (ResetManager)**: 사용자가 버튼을 클릭하면 과거 데이터를 통계 노트로 아카이빙하고 새로운 스케줄 뷰를 렌더링합니다.
...
flowchart TD
    ...
    User([사용자 클릭]) -.-> ResetManager{Reset Manager}
    ResetManager --> |전날 할 일 통계 스캐닝| DailyNote
    ResetManager --> |월간 백업| Archive(월간 통계 아카이브 노트)
```

**💡 4. 사용자 메뉴얼 및 가이드**
- **초기 셋업**: 플러그인 설정 탭에서 `[PARA 구조 생성]` 및 `[일간 스케줄 노트 자동 생성]` 버튼을 눌러 지식 관리 기반을 즉시 구축합니다.
...
- 할 일을 입력할 때 마감일(📅)을 지정하면, 디데이를 계산하여 시각적 마커(`[D]`, `[!]` 등)와 함께 **페이딩(Fading) 배경 하이라이트**로 예쁘게 시각화합니다. (기존의 복잡한 세로 컬러바 대신 심플하고 세련된 디자인으로 통일했습니다)
...
**5) 🔄 데일리 리셋 및 통계 아카이빙**
- 스케줄 노트 상단의 버튼을 클릭하면, 전날의 스케줄 기록을 **월간 통계 노트**로 안전하게 백업합니다.
...
- **기능 구현:** 사용자가 리셋 버튼을 클릭하면, 과거 스케줄을 스캐닝하여 루틴 완료 통계를 산출한 뒤 월간 아카이브 노트로 백업(Append)하고 데일리 노트를 초기화하는 LifeCycle 매니저를 구축했습니다.
