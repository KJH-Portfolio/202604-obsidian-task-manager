...
mainTs = mainTs.replace(
    /const tasks = Array\.from\(element\.querySelectorAll\("\.task-list-item"\)\) as HTMLElement\[\];\r?\n\s*if \(!tasks\.length\) return;/,
...
const startIndex = mainTs.indexOf('                    nodesToProcess.forEach(textNode => {');
const endIndex = mainTs.indexOf('frag.appendChild(dateSpan);\n                        if (after) frag.appendChild(doc.createTextNode(after));\n                        textNode.parentNode?.replaceChild(frag, textNode);\n                    });');
const endOffset = endIndex + 'frag.appendChild(dateSpan);\n                        if (after) frag.appendChild(doc.createTextNode(after));\n                        textNode.parentNode?.replaceChild(frag, textNode);\n                    });'.length;
...
const buttonIndex = mainTs.indexOf(`                        if (taskTextSpan) {
                            taskTextSpan.appendChild(btn);
                        } else {
                            const checkbox = taskEl.querySelector("input[type='checkbox']");`);
const buttonEndIndex = mainTs.indexOf('                            }\n                        }', buttonIndex);
const buttonEndOffset = buttonEndIndex + '                            }\n                        }'.length;

if (buttonIndex !== -1 && buttonEndIndex !== -1) {
...
const calStartIndex = calTs.indexOf(`                const { isMatch, dateStr, lineNo } = isDateClickableRange(view, pos);`);
const calEndIndex = calTs.indexOf(`                    return true;\n                }`);
const calEndOffset = calEndIndex + `                    return true;\n                }`.length;

if (calStartIndex !== -1 && calEndIndex !== -1) {
