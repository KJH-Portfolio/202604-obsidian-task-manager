...
export class TemplateHelper {
    ...
    private readonly scRenderJsContent = `// 1. 프로젝트 파일 가져오기 ("1. Project/01.List" 폴더)
...
`;
    ...
    async createDefaultTemplatesFolderAndFiles(templatesDir: string): Promise<void> {
        const scRenderPath = `${templatesDir}/02.scRender.js`;
        if (!this.app.vault.getAbstractFileByPath(scRenderPath)) {
            await this.app.vault.create(scRenderPath, this.scRenderJsContent);
        }
